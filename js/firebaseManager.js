...
var firebaseManager = {
  ...
  getData: function () {
    return firebase.database().ref('users').child(firebaseManager.username).once('value').then(function(snapshot) {
      data = snapshot.val();
  ...
  deletePost: function(locationID, postID) {
    if (data[locationID].posts[postID] && Object.keys(data[locationID].posts).length <= 1) {
      this.database.ref('users').child(firebaseManager.username).child(locationID).remove();
    } else {
      this.database.ref('users').child(firebaseManager.username).child(locationID).child('posts').child(postID).remove();
    }
  }
}
